...
export function PersonCard({
  ...
  if (category === "tab") {
    if (isArchived) {
      rightSideContent = (
        <div className="text-right">
          <p className="text-emerald-500 font-bold">₹{totalAmount.toFixed(2)}</p>
      ...
      rightSideContent = (
        <div className="text-right">
          <p className="text-emerald-500 font-bold">₹0.00</p>
          <p className="text-[10px] text-emerald-500 uppercase tracking-widest font-bold">
            Balance
          </p>
        </div>
      ...
      rightSideContent = (
        <div className="text-right">
          ...
          <p className="text-[10px] text-slate-600 italic">Partially Paid</p>
        </div>
      );
    } else {
      rightSideContent = (
    ...
    rightSideContent = (
      <p className="text-orange-400 font-bold">₹{totalAmount.toFixed(2)}</p>
    );
  } else {
    rightSideContent = (
      <p className="text-sky-400 font-bold">₹{totalAmount.toFixed(2)}</p>
    );
  }
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <div className="bg-[#121212] border-t border-white/5 py-4 px-4 space-y-4">
          {category === "tab" ? (
              ...
              <div className="flex gap-2">
                {balance > 0 && !isArchived && (
                  <button
                    ...
                    className="text-[10px] font-bold tracking-wider px-3 py-1.5 rounded-lg bg-orange-500/10 text-orange-500 border border-orange-500/20 hover:bg-orange-500 hover:text-white transition-colors"
                  >
                    + ADD PAYMENT
                  </button>
                )}
                {balance <= 0 && !isArchived && (
                  <button
                    ...
                    className="text-[10px] font-bold tracking-wider px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 hover:bg-emerald-500 hover:text-white transition-colors"
                  >
                    SETTLE UP
                  </button>
                )}
                {isArchived && (
                  <button
                    ...
                    className="text-[10px] font-bold tracking-wider px-3 py-1.5 rounded-lg bg-slate-500/10 text-slate-400 border border-slate-500/20 hover:bg-slate-500 hover:text-white transition-colors"
                  >
                    RESTORE
                  </button>
                )}
          ...
          <div className="space-y-3 relative before:absolute before:inset-y-0 before:left-3 before:w-px before:bg-white/5 pl-8">
            {entries.map((entry) => {
              return (
                <div key={entry.id} className="relative">
                  {/* Timeline dot */}
                  <div className="absolute -left-6 top-2.5 w-1.5 h-1.5 rounded-full bg-slate-600 border border-[#121212]"></div>
                  ...
                  <div className="bg-[#1a1a1a] p-3 rounded-xl border border-white/5">
                    ...
                    {entry.note && (
                      <p className="text-[11px] text-slate-400 italic mt-2 opacity-80">
                        "{entry.note}"
                      </p>
                    )}
                  </div>
